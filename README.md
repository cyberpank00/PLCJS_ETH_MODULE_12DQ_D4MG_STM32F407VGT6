...
| Сетевой режим по умолчанию | статический (DHCP выключен) |
| Modbus TCP порт по умолчанию | 502 |
| Modbus Unit ID по умолчанию | 1 |
| Статический IPv4 по умолчанию | `192.168.1.10/24` |
...
|---|---|
| DHCP | выключен (`0`) |
| Статический IP | `192.168.1.10` |
| Маска | `255.255.255.0` |
| Шлюз | `192.168.1.1` |
| TCP-порт | `502` |
| Unit ID | `1` |

По умолчанию используется статический адрес `192.168.1.10`. Для включения DHCP установите регистр `116` в `1`; для смены статических параметров заполните регистры `104…115` при `116 = 0`. Изменения применяются на лету после сохранения через регистр `117` — перезагрузка не требуется (регистр `118` доступен, но для смены IP не нужен).
