...
- индивидуальное и групповое управление выходами по Modbus TCP;
...
2. По умолчанию устройство запрашивает адрес по DHCP.
3. Найдите выданный адрес в DHCP-сервере по MAC-адресу вида `02:00:xx:xx:xx:xx`.
4. Подключитесь Modbus TCP-клиентом к найденному адресу, порту `502`, Unit ID `1`.
...
| Сетевой режим по умолчанию | DHCP |
| Modbus TCP порт по умолчанию | 502 |
| Modbus Unit ID по умолчанию | 1 |
| Статический IPv4 по умолчанию | `192.168.142.147/24` |
| Шлюз по умолчанию | `192.168.142.1` |
...
| FW version образа | `0x0101` (1.1) |
| Версия в Modbus IR120/IR121 | 1.0 |
...
|---|---|
| DHCP | включён (`1`) |
| Статический IP | `192.168.142.147` |
| Маска | `255.255.255.0` |
| Шлюз | `192.168.142.1` |
| TCP-порт | `502` |
| Unit ID | `1` |

При включённом DHCP статические параметры сохраняются как резервная конфигурация, но не применяются. Для перехода на статический адрес установите регистр `116` в `0`, заполните регистры `104…115`, сохраните настройки через регистр `117` и перезагрузите устройство через регистр `118`.
...
Coils и discrete inputs не реализованы. Недопустимый адрес возвращает `ILLEGAL_DATA_ADDRESS`, недопустимое значение — `ILLEGAL_DATA_VALUE`.
...
| `121` | `FW_VERSION_MINOR` | `uint16` | minor-версия интерфейса Modbus, сейчас `0` |
...
| `106` | `IP_OCTET_3` | R/W | `0…255` | `142` | после сохранения и перезагрузки |
| `107` | `IP_OCTET_4` | R/W | `0…255` | `147` | после сохранения и перезагрузки |
...
| `114` | `GW_OCTET_3` | R/W | `0…255` | `142` | после сохранения и перезагрузки |
| `115` | `GW_OCTET_4` | R/W | `0…255` | `1` | после сохранения и перезагрузки |
| `116` | `USE_DHCP` | R/W | `0` static, `1` DHCP | `1` | после сохранения и перезагрузки |
...
node Tools/dq_di_loopback_test.mjs `
  --do-ip 192.168.142.52 `
  --di-ip 192.168.142.88 `
  --edges 100
```
